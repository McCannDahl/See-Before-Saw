...
public class DayPickerLayout : DatePickerLayout  {
	...
	public override void setupCells(DateTime now, Calendar.ActivePanel activePanel) {
		...
		foreach (Button go in CalendarCells) {
			...
			i++;
			go.transform.GetChild(1).localScale = new Vector3(3000, 3000, 3000); // McCann added this
			go.transform.GetChild(1).gameObject.SetActive(activePanel == Calendar.ActivePanel.day);
		}
	}
